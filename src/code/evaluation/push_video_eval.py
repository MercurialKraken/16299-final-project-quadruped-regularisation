"""Record a video of a single robot being pushed laterally.
   Same skeleton as push_recovery_eval.py but with --num_envs=1 and a video wrapper."""
...
parser.add_argument("--mag", type=float, default=350.0)
...
parser.add_argument("--video_length", type=int, default=350)
...
def main(env_cfg, agent_cfg):
    ...
    forces_push[:, base_id, 1] = args_cli.mag
    ...
    wall = time.time() - t0
    print(f"[RESULT] wall={wall:.1f}s  fell={fell}  controller={'smoothed' if args_cli.t_end > 0 else 'raw'}",
          flush=True)
    env.close()
