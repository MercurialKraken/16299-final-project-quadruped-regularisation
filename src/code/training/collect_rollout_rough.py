"""
collect_rollout_rough.py  -- Collect joint/action data from trained Go1 ROUGH terrain policy.
Usage: _isaac_sim\python.bat collect_rollout_rough.py --task Isaac-Velocity-Rough-Unitree-Go1-v0 --headless --num_envs 1 --num_episodes 5
"""
...
OUT_FILE = r"A:\AllIsaac\IsaacLab\rollout_data_rough.npz"
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    try:
        ...
        obs_dim = 235
    print(f"\n[collect-rough] Loaded {CHECKPOINT}")
    print(f"[collect-rough] obs_dim={obs_dim}, Running {args_cli.num_episodes} episodes\n")
    ...
    for ep in range(args_cli.num_episodes):
        ...
        while ep_len < args_cli.max_steps:
            ...
            # Joint pos/vel are at same indices for rough terrain obs too
    ...
    np.savez(OUT_FILE, obs=obs_c, actions=act_c, joint_pos=jpos_c, joint_vel=jvel_c,
             joint_acc=jacc_c, base_vel=bvel_c,
             ep_returns=np.array(ep_returns), ep_lengths=np.array(ep_lengths))
    print(f"[collect-rough] Saved -> {OUT_FILE}")
    env.close()
